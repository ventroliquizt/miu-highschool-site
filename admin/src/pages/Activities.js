...
export default function Activities() {
  ...
  const [data, setData] = useState({
    ...
    subheading: "Enriching experiences beyond the classroom",
  ...
  async function load() {
    ...
    try {
      ...
      setData({
        heading: json?.heading ?? "Extracurricular Activities",
        subheading: json?.subheading ?? "Enriching experiences beyond the classroom",
  ...
  async function save() {
    if (!canSave) {
      setError("Fill at least the section title, and keep at least 1 activity card.");
      return;
    }
    ...
    try {
      ...
      setError(e.message || "Failed to save activities");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1000 }}>
      <h2 style={{ marginBottom: 6 }}>Activities</h2>
      <p style={{ marginTop: 0, color: "#666" }}>
        Edit the “Extracurricular Activities” section on Programs page.
      </p>
      ...
      >
        ...
        <input
          ...
          style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #ccc" }}
          placeholder="Enriching experiences beyond the classroom"
        />
      </div>
        ...
        {data.items.map((item, idx) => (
          ...
          >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
              ...
              <div>
                <label style={{ display: "block", fontWeight: 600, marginBottom: 6 }}>Time</label>
                <input
                  value={item.time}
                  onChange={(e) => updateItem(idx, "time", e.target.value)}
                  style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #ccc" }}
                  placeholder="Wed 3:40-5:00 PM"
                />
              </div>

              <div>
                <label style={{ display: "block", fontWeight: 600, marginBottom: 6 }}>Grades</label>
                <input
                  value={item.grades}
                  onChange={(e) => updateItem(idx, "grades", e.target.value)}
                  style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #ccc" }}
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <textarea
                ...
                placeholder="Develop athletic skills and teamwork."
              />
            </div>
          </div>
        ))}
